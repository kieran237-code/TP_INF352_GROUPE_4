...
* lancement de la bd virtuelle (docker mysql) :
	ps : toujours vérifier la présence du docker-compose.yml  à la racine du projet.
  	...
  	Si en lancant sudo docker compose up -d vous avez une erreur tels que le container mysql-propelize n est pas construis parfaitement vous devez d abord le supprimez avec : sudo docker stop mysql-propelize puis sudo docker rm mysql-propelize  valider et relancer sudo docker compose up -d
